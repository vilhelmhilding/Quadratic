...
def build_pair(p1, p2, log_ret, window):
    x = log_ret[p2]
    y = log_ret[p1]
    beta = (y.rolling(window).cov(x) / x.rolling(window).var()).replace([np.inf, -np.inf], np.nan)
    # residual = y - beta * x
    ...
    z = ((spread - spread.rolling(window).mean()) / spread.rolling(window).std()).fillna(0)
    return (p1, p2, beta, z)
...
def target_strategy_1(row_pos, data):
    ...
    for i, (p1, p2, beta, z) in enumerate(pairs):
        ...
        # ENTRY
        if state == 0:
            if p1 == 'Idx_01':   # asymmetrisk bara för Idx_01-strategier (blanka vix är farligt )
                if z_t > 2.5:
                    state = -1
                elif z_t < -1.0:
                    state = 1
            else:                # symmetrisk för övriga
                if z_t > 1.0:
                    state = -1
                elif z_t < -1.0:
                    state = 1
...
# bygg A en gång om du vill hedgea med FX
...
# Virtuell sleeve för strategi 2
...
def target_strategy_2(row_pos, cash, portfolio, signal_prices, data):
    ...
    # Mellan rebalanseringar: behåll strategi 2:s egna targets
    ...
    total_value = sleeve_2_cash + sum(
        sleeve_2_positions.get(t, 0) * signal_prices.get(t, 0)
    ...
    # =========================
    # INDEX HEDGE (Idx_04) - beta-justerad
    # =========================
    idx_ticker = "Idx_04"
    ...
    if px_idx > 0:
        ...
        for ticker in STOCK_COLS:
            ...
            beta = (
                stk_ret.rolling(LOOKBACK).cov(idx_ret) /
                idx_ret.rolling(LOOKBACK).var()
            ).loc[date]
        ...
        # Hedge beta-exponeringen med index
        idx_shares = int(- total_beta_notional / px_idx)
        targets[idx_ticker] = idx_shares

    # # FX-hedge: exakt som originalet
    # A = np.zeros((len(currencies), len(fx_list)))
    # for j, fx_ticker in enumerate(fx_list):
    #     c1, c2 = fx_pairs_map[fx_ticker]
    #     px = signal_prices.get(fx_ticker, 0)
    #     if px <= 0:
    #         continue
    #     A[currencies.index(c1), j] =  px
    #     A[currencies.index(c2), j] = -px

    # b = np.array([ccy_exposure[c] for c in currencies], dtype=float)
    # rhs = b.mean() - b
    # x, *_ = np.linalg.lstsq(A, rhs, rcond=None)

    # for j, fx_ticker in enumerate(fx_list):
    #     targets[fx_ticker] = int(round(x[j]))

    # Uppdatera strategi 2:s interna sleeve-cash som om handeln genomförs
    trade_cash = 0.0
...
# Virtuell sleeve för strategi 3
sleeve_3_cash = sleeve_3_cash0
sleeve_3_positions = {}

def _equity_3(cash, positions, signal_prices):
    return cash + sum(positions.get(t, 0) * signal_prices.get(t, 0) for t in prices.columns)

def _current_targets_3(positions):
    return {t: positions.get(t, 0) for t in [ETF_3] + STOCKS_3 if positions.get(t, 0) != 0}
...
def target_strategy_3(row_pos, signal_prices, data):
    ...
    if not np.isfinite(px_etf) or px_etf <= 0 or px_stk.isna().any() or (px_stk <= 0).any():
        ...
        targets = _cap_targets_to_cash_3(targets, sleeve_3_cash, sleeve_3_positions, signal_prices)
...
def apply_portfolio_fx_hedge(total_targets, signal_prices):
    ...
    # Nollställ alla FX-targets först
    for fx in FX_COLS:
        targets[fx] = 0

    # Summera hela portföljens foreign currency exposure
    exp_02 = 0.0
    exp_03 = 0.0
    exp_04 = 0.0
    ...
    # Hedge direkt mot base currency
    px_fx01 = signal_prices.get('FX_01', 0)   # (Crncy_02, Crncy_01)
    px_fx05 = signal_prices.get('FX_05', 0)   # (Crncy_01, Crncy_03)
    px_fx06 = signal_prices.get('FX_06', 0)   # (Crncy_04, Crncy_01)

    if px_fx01 > 0:
        # FX_01 holding q ger +q*P i Crncy_02
        targets['FX_01'] = int(round(-exp_02 / px_fx01))

    if px_fx05 > 0:
        # FX_05 holding q ger -q*P i Crncy_03
        targets['FX_05'] = int(round(exp_03 / px_fx05))

    if px_fx06 > 0:
        # FX_06 holding q ger +q*P i Crncy_04
...
def strategy(row_pos, cash, portfolio, signal_prices, data):
    ...
    # Lägg FX-hedgen sist på totalportföljen
